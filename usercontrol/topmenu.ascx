<%@ Control Language="C#" AutoEventWireup="true" CodeFile="topmenu.ascx.cs" Inherits="admin_usercontrol_topmenu" %>
...
$(function() {
    ...
    $(window).scroll(function() {
        if ($(this).scrollTop() > pos.top + menu.height() && menu.hasClass('dropmenu')) {
            menu.fadeOut('fast', function() {
                $(this).removeClass('dropmenu').addClass('fixed').fadeIn('fast');
            });
        } else if ($(this).scrollTop() <= pos.top && menu.hasClass('fixed')) {
            menu.fadeOut('fast', function() {
                $(this).removeClass('fixed').addClass('dropmenu').fadeIn('fast');
            });
        }
    });
});
...
<%--<div class="menu_container">	
    <ul id="topnav" style="line-height: 30px">
        <%=strmenus%>
...
<div class="menu_container1">	
    <ul id="nav-one" class="dropmenu">
        <%=strmenus%>
        <li class="haschildren"><a href="../logout.aspx">Logout</a></li>
    </ul>
</div>
